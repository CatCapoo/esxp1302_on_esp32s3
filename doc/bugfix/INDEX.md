...
| [BUG-010](BUG-010_push_data_ack_low_out_of_sync.md) | 2026-02-21 | `main/packet_forwarder/lora_pkt_fwd.c` | `sock_up/sock_down` 未 `connect()`，`recvfrom` 覆盖全局 `dest_addr`；`PUSH_TIMEOUT_MS=100ms` 远小于 NS RTT（~400ms）；两者导致 out-of-sync ACK、PUSH_DATA acknowledged 低至 22% | 功能性错误 |
