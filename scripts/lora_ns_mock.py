...
#   默认端口 1700（LoRa packet forwarder 标准端口）
#
# 示例:
...
def run(port: int):
    ...
    pull_addr = None  # 记录网关发 PULL_DATA 的地址，供将来发 PULL_RESP 用
...
if __name__ == "__main__":
    port = int(sys.argv[1]) if len(sys.argv) > 1 else 1700
    run(port)
