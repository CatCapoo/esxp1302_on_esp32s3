...
class E77Node:
    ...
    # ── 高层指令 ──────────────────────────────
    def send_cmd(self, cmd: str, expect="OK", timeout=DEFAULT_TIMEOUT) -> tuple[bool, list[str]]:
    ...
    def config_otaa(self, region: int, deveui: str, appeui: str, appkey: str,
        ...
        if chanmask:
            steps.append((f"AT+CMANUALMASK=1",              "手动掩码使能"))
...
def build_parser():
    ...
    p.add_argument("command", choices=["otaa", "abp", "query", "send", "restore"],
                   help="执行的动作")
    p.add_argument("--port",     required=True,       help="串口设备，如 /dev/ttyUSB0")
    p.add_argument("--baud",     type=int, default=9600, help="波特率（默认 9600）")
    p.add_argument("--verbose",  action="store_true", help="打印所有串口收发原始内容")

    # LoRaWAN 公共参数
    p.add_argument("--region",   type=int, default=2,
                   help="频段 ID：0=AS923 1=AU915 2=CN470 4=EU433 5=EU868 8=US915（默认 2=CN470）")
    p.add_argument("--adr",      type=int, default=1,  help="ADR 0=关 1=开（默认 1）")
    ...
    # OTAA 参数
    p.add_argument("--deveui",   default=None, help="OTAA: DevEUI（16位hex）")
    p.add_argument("--appeui",   default="0000000000000000", help="OTAA: AppEUI（默认全0）")
    p.add_argument("--appkey",   default=None, help="OTAA: AppKey（32位hex）")
    # ABP 参数
    p.add_argument("--devaddr",  default=None, help="ABP: DevAddr（8位hex，如 26011234）")
    p.add_argument("--nwkskey",  default=None, help="ABP: NwkSKey（32位hex）")
    p.add_argument("--appskey",  default=None, help="ABP: AppSKey（32位hex）")
    # 发送参数
