...
int stts751_configure(uint8_t i2c_addr)
{
    ...
    err = lgw_i2c_read(i2c_addr, STTS751_REG_PROD_ID, &val);
    if (err != 0) {
    ...
    /* Get Manufacturer ID */
    err = lgw_i2c_read(i2c_addr, STTS751_REG_MAN_ID, &val);
    if (err != 0) {
        DEBUG_PRINTF("ERROR: failed to read I2C device 0x%02X (err=%i)\n", i2c_addr, err);
        return LGW_I2C_ERROR;
    }
    ...
    /* Get revision number */
    err = lgw_i2c_read(i2c_addr, STTS751_REG_REV_ID, &val);
    if (err != 0) {
        DEBUG_PRINTF("ERROR: failed to read I2C device 0x%02X (err=%i)\n", i2c_addr, err);
        return LGW_I2C_ERROR;
    }
    ...
    err = lgw_i2c_write(i2c_addr, STTS751_REG_CONF, 0x8C); /* TODO: do not hardcode the whole byte */
    if (err != 0) {
    ...
    err = lgw_i2c_write(i2c_addr, STTS751_REG_RATE, 0x04);
    if (err != 0) {
        DEBUG_PRINTF("ERROR: failed to write I2C device 0x%02X (err=%i)\n", i2c_addr, err);
        return LGW_I2C_ERROR;
    }
...
int stts751_get_temperature(uint8_t i2c_addr, float * temperature)
{
    ...
    err = lgw_i2c_read(i2c_addr, STTS751_REG_TEMP_L, &low_byte);
    if (err != 0) {
    ...
    err = lgw_i2c_read(i2c_addr, STTS751_REG_TEMP_H, &high_byte);
    if (err != 0) {
        printf("ERROR: failed to read I2C device 0x%02X (err=%i)\n", i2c_addr, err);
        return LGW_I2C_ERROR;
    }
