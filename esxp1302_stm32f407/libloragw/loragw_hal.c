...
static bool is_same_pkt(struct lgw_pkt_rx_s *p1, struct lgw_pkt_rx_s *p2) {
    if ((p1 != NULL) && (p2 != NULL)) {
        /* Criterias to determine if packets are identical:
            -- count_us should be equal or can have up to 24碌s of difference (3 samples)
...
/* qsort_r replacement for newlib (bare-metal ARM) */
static int *_qsort_swap_counter = NULL;

static int compare_pkt_tmst_wrap(const void *a, const void *b)
{
    ...
    if (p_count > q_count && _qsort_swap_counter != NULL) {
        *_qsort_swap_counter += 1;
    }
...
static int merge_packets(struct lgw_pkt_rx_s * p, uint8_t * nb_pkt) {
    ...
    _qsort_swap_counter = &counter_qsort_swap;
    qsort(p, cpt, sizeof(p[0]), compare_pkt_tmst_wrap);
    _qsort_swap_counter = NULL;
